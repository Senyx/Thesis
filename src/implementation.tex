...
% autoposun, posuvnik, prekryv delayu, obecně pro libovnoé metody, runwaye, weight classes
...
\node [term, fill=black!20]	{\ding{172} Delay all updated slots};
...
\end{tikzpicture}
    \caption{\red{Example of runway plan with colliding slots}}
    \label{fig:TEST}
\end{figure}
...
\path (x3.south) to node [near start, xshift=2.3em] {$d \geq 3$min} (t1);
	\draw [*->,black] (x3.south) -- (t1);
\path (x3.east) to node [near start, yshift=1em, xshift=0.6em] {$d > 3$min} (c3); 
